...
load("2_data/case_study/01_monkey/01_bulk_rna_seq/dt_d.rda")
...
variable_info <- convert_id(data = dt_d,
                            query_type = "gene",
                            from_id_type = "ensembl",
                            ah_id = "AH119900",
                            return_orgdb = TRUE)
variable_info$data$Symbol[variable_info$data$Symbol == "NA"] <- NA
sum(is.na(variable_info$data$Symbol)) < sum(is.na(variable_info$data$symbol))
variable_info$data <- variable_info$data |> dplyr::mutate(symbol = Symbol) |> dplyr::select(-Symbol)
save(variable_info, file = "01_bulk_rna_seq_down_variable_info.rda")
...
save(enrich_pathway_res,
     file = "01_bulk_rna_seq_up_enrich_pathway_res_cluster_d.rda")
...
save(embedsim_res, file = "01_bulk_rna_seq_down_embedsim_res.rda")
...
save(optimal_clustering_test, file = "01_bulk_rna_seq_down_optimal_clustering_test_biotext.rda")
optimal_clustering_test$evaluation_plot
optimal_clustering_test$best_combination # use louvain, cutoff: 0.45

# optimal: hierarchical_ward.D2 0.85 | louvain 0.8
load("01_bulk_rna_seq_down_embedsim_res.rda")
functional_module_res <- get_functional_modules(
  object = embedsim_res,
  sim.cutoff = 0.55,
...
save(functional_module_res, file = "01_bulk_rna_seq_down_functional_module_res_biotext.rda")

## Step4: LLM interpretation ====
setwd("3_data_analysis/05_case_study/01_monkey/")
load("01_bulk_rna_seq_down_functional_module_res_biotext.rda")
ah <- AnnotationHub::AnnotationHub()
# ### Query the object for Macaca fascicularis
# mf_query_result <- AnnotationHub::query(ah, c("OrgDb", "9541"))
# mf_query_result$ah_id
...
setwd(get_project_wd())
save(llm_interpreted_fm_res, file = "2_data/case_study/01_monkey/05_llm_interpreted_result/01_bulk_rna_seq_down_llm_interpreted_fm_res.rda")

## Step5: Result visualization ====
load("2_data/case_study/01_monkey/05_llm_interpreted_result/01_bulk_rna_seq_down_llm_interpreted_fm_res.rda")
setwd("3_data_analysis/05_case_study/01_monkey/")
...
library(Cairo)
CairoPDF("result_visualization/01_bulk_rna_seq_down_functional_module_network.pdf", width = 12, height = 8)
plot
dev.off()
