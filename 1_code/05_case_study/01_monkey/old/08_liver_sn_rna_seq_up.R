...
load("2_data/case_study/01_monkey/04_sn_rnaseq/liver_up_dt_converted.rda")
...
liver_up_dt <- liver_up_dt_converted |>
  group_by(Gene) |>
  arrange(p_val_adj) |>
  ungroup() |>
  distinct(Gene, .keep_all = TRUE)

liver_up_dt <- liver_up_dt |> dplyr::rename(symbol = Gene)

variable_info <- convert_id(data = liver_up_dt,
                            query_type = "gene",
                            from_id_type = "symbol",
                            ah_id = "AH119900",
                            return_orgdb = TRUE)
...
save(variable_info_data, file = "04_sn_rna_seq_liver_up_variable_info.rda")
...
save(enrich_pathway_res,
     file = "04_sn_rna_seq_liver_up_enrich_pathway_res.rda")
...
save(embedsim_res, file = "04_sn_rna_seq_liver_up_embedsim_res.rda")
...
save(optimal_clustering_test, file = "04_sn_rna_seq_liver_up_optimal_clustering_test_biotext.rda")
optimal_clustering_test$evaluation_plot
optimal_clustering_test$best_combination # use louvain, cutoff: 0.88

# optimal: louvain 0.84
load("04_sn_rna_seq_liver_up_embedsim_res.rda")
...
save(functional_module_res, file = "04_sn_rna_seq_liver_up_functional_module_res_biotext.rda")

## Step4: LLM interpretation ====
setwd("3_data_analysis/05_case_study/01_monkey/")
load("04_sn_rna_seq_liver_up_functional_module_res_biotext.rda")
ah <- AnnotationHub::AnnotationHub()
mf.orgdb <- ah[["AH119900"]] #| Taxonomy ID: 9541
...
setwd(get_project_wd())
save(llm_interpreted_fm_res, file = "2_data/case_study/01_monkey/05_llm_interpreted_result/04_sn_rna_seq_liver_up_llm_interpreted_fm_res.rda")

## Step5: Result visualization ====
load("2_data/case_study/01_monkey/05_llm_interpreted_result/04_sn_rna_seq_liver_up_llm_interpreted_fm_res.rda")
setwd("3_data_analysis/05_case_study/01_monkey/")
...
library(Cairo)
CairoPDF("result_visualization/04_sn_rna_seq_liver_up_functional_module_network.pdf", width = 12, height = 8)
plot
dev.off()
