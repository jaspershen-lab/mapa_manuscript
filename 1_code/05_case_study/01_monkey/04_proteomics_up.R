...
# load("2_data/case_study/01_monkey/02_proteomics/p_dt_cleaned_up.rda")
...
p_dt_cleaned_up <- p_dt_cleaned_up |> dplyr::rename(symbol = ID)
variable_info <- convert_id(data = p_dt_cleaned_up,
                            query_type = "gene",
                            from_id_type = "symbol",
                            ah_id = "AH119900",
                            return_orgdb = TRUE)
variable_info_data <- variable_info$data
save(variable_info_data, file = "02_proteomics_up_variable_info.rda")
...
save(enrich_pathway_res,
     file = "02_proteomics_up_enrich_pathway_res.rda")
...
# load("01_bulk_rna_seq_up_enrich_pathway_res_cluster_u.rda")
enrich_pathway_res@enrichment_go_result@result <- enrich_pathway_res@enrichment_go_result@result |> mutate(p_adjust = pvalue)
enrich_pathway_res@enrichment_kegg_result@result <- enrich_pathway_res@enrichment_kegg_result@result |> mutate(p_adjust = pvalue)
...
save(embedsim_res, file = "02_proteomics_up_embedsim_res.rda")
...
save(optimal_clustering_test, file = "02_proteomics_up_optimal_clustering_test_biotext.rda")
optimal_clustering_test$evaluation_plot
optimal_clustering_test$best_combination # use louvain, cutoff: 0.6

# optimal: hierarchical_ward.D2 0.85 | louvain 0.8
load("02_proteomics_up_embedsim_res.rda")
functional_module_res <- get_functional_modules(
  object = embedsim_res,
  sim.cutoff = 0.6,
  cluster_method = "louvain"
...
save(functional_module_res, file = "02_proteomics_up_functional_module_res_biotext.rda")

## Step4: LLM interpretation ====
setwd("3_data_analysis/05_case_study/01_monkey/")
load("02_proteomics_up_functional_module_res_biotext.rda")
ah <- AnnotationHub::AnnotationHub()
mf.orgdb <- ah[["AH119900"]] #| Taxonomy ID: 9541
...
setwd(get_project_wd())
save(llm_interpreted_fm_res, file = "2_data/case_study/01_monkey/05_llm_interpreted_result/02_proteomics_up_llm_interpreted_fm_res.rda")
...
library(Cairo)
CairoPDF("result_visualization/02_proteomics_up_functional_module_network.pdf", width = 12, height = 8)
plot
dev.off()
